...
class TraceableSsdModule(torch.nn.Module):
  ...
  def create_anchors(self, output_tuple):
    """Create anchors by analyzing the shape of bbox_head outputs.

    :param output_tuple: Output generated by bbox_head
    :return:
    """
    feat_map_size = [output_tuple[i].shape[-2:] for i in range(6)]
    anchors = self.bbox_head.anchor_generator.grid_anchors(feat_map_size)

    return anchors

  def convert_multi_level_output_to_tvm_format(self, output_tuple):
    anchors = self.create_anchors(output_tuple)
    multi_level_cls_probs = []
    multi_level_loc_preds = []
    multi_level_anchors = []

    for level in range(6):
      cls_score = output_tuple[level]
      bbox_pred = output_tuple[level + 6]
    ...
    multi_level_cls_probs = np.concatenate(multi_level_cls_probs, axis=2)
    multi_level_loc_preds = np.concatenate(multi_level_loc_preds, axis=1)
    multi_level_anchors = np.concatenate(multi_level_anchors, axis=1)

    return multi_level_cls_probs, multi_level_loc_preds, multi_level_anchors

  def convert_output_to_tvm_format(self, cls_score, bbox_pred, anchor):
    ...
    cls_score = torch.cat((cls_score[:, [self.NUM_CLASSES]], cls_score[:, 0:self.NUM_CLASSES]), axis=1)
    ...
    # convert to numpy
    np_cls_prob = cls_score.detach().numpy()
    np_loc_preds = bbox_pred.detach().numpy()
    np_anchors = anchor.cpu().numpy()

    return np_cls_prob, np_loc_preds, np_anchors
